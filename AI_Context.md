...
## 🚀 БЫСТРЫЙ СТАТУС (10.01.2026 18:00)
...
| **Yandex Object Storage** | 🟡 Решение выбрано, нужны ключи | 50% |
| **Deploy Backend (VPS)** | 🔴 **КРИТИЧНО!** | 0% |

---

## 🚨 КРИТИЧЕСКИЕ БЛОКЕРЫ (10.01.2026)

### 1️⃣ Deploy Backend на VPS/Railway (P0)

**ПРОБЛЕМА:**
- Frontend на Vercel (motorsoft.pro) ✅
- Backend работает ТОЛЬКО на Mac (localhost:8000) ❌
- Vercel **НЕ МОЖЕТ** подключиться к localhost — это локальный адрес!
- **Админка на motorsoft.pro/admin НЕ РАБОТАЕТ** без этого

**РЕШЕНИЕ:**
1. Выбрать хостинг (Railway бесплатно / VPS ~300₽/мес)
2. Загрузить docker-compose на сервер
3. Настроить PostgreSQL на сервере
4. Получить публичный URL (например `https://api.motorsoft.pro`)
5. Добавить `NEXT_PUBLIC_API_URL=https://api.motorsoft.pro/api/v1` в Vercel
6. Redeploy frontend

**ВАРИАНТЫ ХОСТИНГА:**
| Хостинг | Цена | Комментарий |
|---------|------|-------------|
| **Railway.app** | БЕСПЛАТНО | Для теста, лимиты по трафику |
| **Timeweb Cloud** | от 199₽/мес | Россия, быстро |
| **Selectel** | от 300₽/мес | Россия, надёжно |
| **Hetzner** | €4/мес | Европа, дёшево |
| **DigitalOcean** | $6/мес | Надёжно |

### 2️⃣ Yandex Object Storage (P0) — РЕШЕНИЕ ВЫБРАНО ✅

**У клиента уже есть Yandex Cloud** (используется для OCR Vision API).

**Тариф: Standard** (НЕ Cold/Ледяной!)
- Cold дешевле хранить, но дорого читать + штрафы за раннее удаление

**Стоимость (~25₽/мес):**
| Статья | Расчёт | Цена |
|--------|--------|------|
| Хранение 11 ГБ | 7,410 файлов × 1.5 МБ × 2.10₽/ГБ | ~23₽/мес |
| Исходящий трафик | FREE TIER первые 100 ГБ | 0₽/мес |
| Запросы GET/PUT | копейки | <2₽/мес |
| **ИТОГО** | | **~25₽/мес** |

**Настройка:**
1. Создать бакет `motorsoft-firmwares` (Standard, Private)
2. IAM → Сервисный аккаунт → Роль `storage.editor`
3. Создать статический ключ (Access Key ID + Secret Access Key)
4. Интегрировать boto3 в backend
...
- ❌ ACCESS_KEY_ID: нужно создать статический ключ
- ❌ SECRET_ACCESS_KEY: нужно создать статический ключ

---

**Текущий этап:** 
1. ✅ Admin Panel MVP полностью завершён (работает локально)
2. 🔴 **КРИТИЧНО:** Deploy Backend на VPS — без этого админка не работает на сайте!
3. 🟡 **Yandex Object Storage** — нужно создать бакет и статический ключ
...
**Следующий этап:** 
1. ~~Admin Panel с дашбордом~~ → ⏳ Ожидание
2. **ПОКУПКА И ВЫДАЧА ПРОШИВОК** → 🔴 КРИТИЧНО!
   - Нужны сами .bin файлы 
   - Нужна система хранилища
   - Нужна автоматическая выдача при покупке

---

## 🛒 ПОКУПКА И ВЫДАЧА ПРОШИВОК (10.01.2026 — КРИТИЧЕСКАЯ ЗАДАЧА)

### ✅ ЧТО УЖЕ РАБОТАЕТ:

**Backend:**
- `POST /orders/create` — создание заказа ✅
- `POST /orders/{order_id}/purchase` — проверка баланса и списание денег ✅
- `GET /orders/{order_id}` — получение информации о заказе ✅

**Бот:**
- Загрузка файла/скриншота → Поиск в базе → Показывает цену ✅
- Кнопка "Купить" → Проверка баланса → Списание денег ✅
- Уведомление оператору при нераспознанном скриншоте ✅

### ❌ ЧТО НЕ РАБОТАЕТ:

**ГЛАВНАЯ ПРОБЛЕМА:** Нет самих файлов прошивок!

```
База данных:
├─ 7,407 записей
│  ├─ software_id: "39101-2F310" ✅
│  ├─ brand: "Hyundai" ✅
│  ├─ series: "Santa Fe 2.2D" ✅
│  └─ file_path: NULL ❌❌❌ ← ПУСТО!
└─ Файлы .bin: ОТСУТСТВУЮТ НА ДИСКЕ ❌❌❌
```

### 🤔 КРИТИЧЕСКИЕ ВОПРОСЫ К КЛИЕНТУ:

#### 1️⃣ **ГДЕ ХРАНИТЬ ФАЙЛЫ?**

**✅ РЕШЕНО: Yandex Object Storage (10.01.2026)**

Клиент уже имеет Yandex Cloud (используется для OCR Vision API).
Object Storage — идеальное решение:

```
Бакет: motorsoft-firmwares
├── 2605.bin           # Hyundai Santa Fe
