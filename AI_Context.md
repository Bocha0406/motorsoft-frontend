...
## 🚀 БЫСТРЫЙ СТАТУС (14.01.2026 — ФИНАЛЬНОЕ ОБНОВЛЕНИЕ!)
...
- `POST /api/v1/api/admin/login` — авторизация
- `GET /api/v1/api/admin/users` — список пользователей
- `GET /api/v1/api/admin/orders` — список заказов
- `GET /api/v1/api/admin/firmwares` — список прошивок
- `POST /api/v1/api/admin/firmwares/upload` — загрузка файла

**Frontend (на Vercel):**
- URL: https://motorsoft.pro/admin
- Страницы: login, users, orders, firmwares, settings

**⚠️ ПРОБЛЕМА:** Frontend на Vercel обращается к localhost или неправильному API URL.

**РЕШЕНИЕ:** Нужно в Vercel добавить переменную:
```
NEXT_PUBLIC_API_URL=http://185.152.92.157:8000
```
